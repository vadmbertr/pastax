    ...
    def from_arrays(
        ...
        Args:
            ...
            lon_closed: Optional override for the closed/open classification.
                ...
                → closed). Pass explicitly to build a grid under ``jit``/``vmap``
                tracing, where concrete coordinates are unavailable and the
                classification would otherwise default to closed. Passing
                ``lon_closed=False`` under tracing additionally unwraps a
                seam-crossing axis for you, so raw seam coordinates can be loaded
                inside ``jit``/``vmap`` without pre-unwrapping. Ignored when
                ``lon_period`` is ``None``.
...
def _classify_and_unwrap_lon(
    ...
    * **open** — a seam-crossing regional slab (e.g. ``170°…-170°``) spanning
    ...
    The *classification* reads concrete values, so under tracing
    (``jit``/``vmap``) it cannot run: the closed/open split then defaults to
    ``closed`` (today's behaviour) unless an explicit ``lon_closed`` override is
    passed. The *unwrap*, however, is pure arithmetic and works under tracing —
    so when the caller declares the grid open (``lon_closed=False``) a seam is
    unwrapped even under a tracer, letting raw seam coords be loaded inside
    ``jit``/``vmap`` without pre-unwrapping.

    Computation uses ``jnp`` and so honours the caller's precision config: under
    the default ``jax_enable_x64=False`` it runs in float32, which the ``1e-3``
    classification tolerance comfortably absorbs. One edge: the seam
    equal-spacing check compares float32 ``diff``s, so an ultra-fine grid at the
    dateline (spacing ``dlon`` at or below ~0.02°, where float32 cancellation
    near 180° approaches the tolerance) may be spuriously rejected — pass
    ``lon_closed=False`` (which skips host-side validation) or pre-unwrap.
    """
    if lon_period is None:
        return lon, True
    if _is_traced(lon):
        # No concrete values → cannot classify or validate (caller owns
        # correctness). The unwrap is traceable, so honour an explicit "open"
        # declaration and unwrap a possible seam; closed/None grids need no
        # monotone axis (the periodic index uses only x0 and dx) and are left
        # untouched, preserving the closed byte-identity.
        if lon_closed is False:
            step = jnp.mod(jnp.diff(lon), lon_period)
            lon = lon[0] + jnp.cumsum(
                jnp.concatenate([jnp.zeros((1,), lon.dtype), step])
            )
            return lon, False
        return lon, True if lon_closed is None else bool(lon_closed)

    lon_j = jnp.asarray(lon)
    nlon = int(lon_j.shape[0])
    if nlon < 2:
        raise ValueError(
            "lon_period requires at least 2 longitude points to define the "
            f"grid spacing; got nlon={nlon}."
        )
    diffs = jnp.diff(lon_j)
    if bool(jnp.all(diffs > 0)):
        lon_out = lon  # already ascending — pass through unchanged (byte-identical)
        dlon = float(lon_j[1] - lon_j[0])
    elif bool(jnp.all(diffs < 0)):
        raise ValueError(
        ...
        step = jnp.mod(diffs, lon_period)
        if not bool(jnp.allclose(step, step[0], rtol=1e-3)):
            raise ValueError(
        ...
        lon_out = lon_j[0] + jnp.cumsum(
            jnp.concatenate([jnp.zeros((1,), lon_j.dtype), step])
        )
        dlon = float(step[0])
